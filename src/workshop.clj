...
(defn insert
  ...
  (let [old-db (read-db)
        ...
        index-key-list (keys (id-key (:indexes (table-name old-db))))]
    (if (some #{(id-key record)} index-key-list )
      (println (str "Record with name " (id-key record) " already exists. Exiting."))
